...
A = df %>%
  ...
  summarize(
    ...
    `First session_months` = round(min(AgeWeeks) / 4.348, 1), # Converting weeks to months
    `Last session_months` = round(max(AgeWeeks) / 4.348, 1),
...
left_join(A, B, by = 'ID_id_num') %>%
  ...
  select(
    ...
    Hammer,
    Magnet
  ) %>% # Manually reorder columns with the spacer
  gt() %>%
  cols_label(
    ID_id_num = "Id",
    `First session_months` = "First session", # Rename to match weeks column
    `Last session_months` = "Last session" # Rename to match weeks column
  ) %>%
  tab_spanner(
    label = "Age (weeks)",
  ...
  tab_spanner(
    label = "Age (months)",
    columns = c(`First session_months`, `Last session_months`)
  ) %>%
...
mod = brm(
  AdaptiveGrasp ~
    Trial_tool * AgeSt * Trial_tool_direction + (1 + AgeSt | ID_id_num),
  data = db,
  family = bernoulli(),
  prior = Priors,
  chains = 4,
  ...
  file = "./Results/Models/ToolUseWeeks"
)
...
estimate_contrasts(
  mod,
  contrast = 'Trial_tool',
  by = 'Trial_tool_direction',
  ci = .89
)

Est_mod = estimate_means(mod, by = c('Trial_tool', 'Trial_tool_direction'), backend = 'emmeans')
Est_mod$Trial_tool_direction = factor(
  Est_mod$Trial_tool_direction,
  levels = c("l", "r"),
...
Pred_Subject = as.data.frame(summary(
  Pred_Subject,
  type = "response",
  level = 0.89
))
...
slopes_Subject_tool <- emtrends(
  mod,
  ~ Trial_tool | ID_id_num,
  var = "AgeSt",
...
slopes_df = slopes_df %>%
  mutate(
    Sign = ifelse(lower.HPD <= 0 & upper.HPD >= 0, F, T),
    Trial_tool = factor(
      Trial_tool,
      levels = c("Spoon", "Hammer", "Brush", "Magnet")
    )
  )
...
slopes_df |> 
  group_by(Trial_tool) |> 
  summarise(
    meanEst = mean(AgeSt.trend),
    meanLow = mean(lower.HPD),
    meanUp = mean(upper.HPD))



plot_sl = ggplot(
  slopes_df,
  aes(x = ID_id_num, y = AgeSt.trend, color = ID_id_num)
) +
...
Age_to_calculate_over = standardize(seq(38, 86, 2), reference = db$Age)

# Create new data
MeansDraws <- expand.grid(
  AgeSt = Age_to_calculate_over,
  Trial_tool = "Spoon",
...
Pred <- MeansDraws %>%
  ...
  mutate(
    Age = unstandardize(AgeSt, reference = db$Age),  # still in weeks
    Age_days = Age * 7,  # convert weeks to days
    years = floor(Age_days / 365.25),
    months = floor((Age_days %% 365.25) / 30.44),
    days = floor((Age_days %% 365.25) %% 30.44),
    TotMonths = Age_days / 30.44
  )
...
Prediction_plot = MeansDraws %>%
  ...
  annotate(
    "rect",
    xmin = Pred[which(Pred$CI_low >= 0.75)[1], ]$Age - 0.5,
    xmax = max(Pred$Age) + 3,
...
gc()
if (file.exists('.\\Results\\Models\\SimualtionWeeks.RDS')) {
  Df = readRDS('.\\Results\\Models\\SimualtionWeeks.RDS')
} else {
  ...
  run_simulation <- function(i, xx) {
    ...
    Db = as.data.frame(estimate_slopes(
      modLm,
      trend = 'AgeSt',
      by = 'Trial_tool'
    ))
  ...
  saveRDS(Df, '.\\Results\\Models\\SimualtionWeeks.RDS')
  gc()
}
...
H = ggplot(Hammer, aes(x = x, y = y)) +
  ...
  geom_area(
    data = Hammer %>% filter(x >= HammerCi$CI_low & x <= HammerCi$CI_high),
    fill = "#D95F02",
    alpha = 1
  ) +
  ...
  geom_area(
    data = HammerP %>% filter(x >= HammerPCi$CI_low & x <= HammerPCi$CI_high),
    fill = "#31688e",
    alpha = 1
  ) +
...
M = ggplot(Magnet, aes(x = x, y = y)) +
  ...
  geom_area(
    data = Magnet %>% filter(x >= MagnetCi$CI_low & x <= MagnetCi$CI_high),
    fill = "#D95F02",
    alpha = 1
  ) +
...
S = ggplot(Spoon, aes(x = x, y = y)) +
  geom_area(fill = "#D95F02", alpha = 0.3) + # Background area
  geom_area(
    data = Spoon %>% filter(x >= SpoonCi$CI_low & x <= SpoonCi$CI_high),
    fill = "#D95F02",
    alpha = 1
  ) +
  ...
  geom_area(
    data = SpoonP %>% filter(x >= SpoonPCi$CI_low & x <= SpoonPCi$CI_high),
    fill = "#440154",
    alpha = 1
  ) +
...
B = ggplot(Brush, aes(x = x, y = y)) +
  geom_area(fill = "#D95F02", alpha = 0.3) + # Background area
  geom_area(
    data = Brush %>% filter(x >= BrushCi$CI_low & x <= BrushCi$CI_high),
    fill = "#D95F02",
    alpha = 1
  ) +
  ...
  geom_area(
    data = BrushP %>% filter(x >= BrushPCi$CI_low & x <= BrushPCi$CI_high),
    fill = "#35b779",
    alpha = 1
  ) +
...
SesXDir = db |> 
  ...
  scale_color_manual(values = my_colors) 
saveRDS(SesXDir, '.\\Results\\Plots\\SessionSpreadPlotToolDirection.rds')
ggsave('.\\Results\\Plots\\SessionSpreadPlotToolDirection.svg',SesXDir,  height = 10, width = 20, dpi = 300)
